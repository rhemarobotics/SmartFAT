 ...
 * SmartFactoryEx3.ino 
 * 目的:
 *  測試智慧工廠範例三功能 - 透過機器人實現智慧工廠物件重量分類應用 
 *  放置物件於輸送帶上,待物件到達輸送帶定位點,被紅外線感測器偵測到後,
 *  會停止輸送帶,並通知樹莓派機器人至定位點位置,夾取物件放置於重量感測器位置
 * 方法:
 *  紅外線感測器 + 8*8矩陣LED + 輸送帶模組 + 三色指示燈 + 蜂鳴器 + 旋轉電位器 + 重量感測器
 * 步驟:
 *  直接執行程式
 * 結果:
 *  1.剛開始,開啟輸送帶運轉
...
#define WH_DT_Pin  4   //定義重量感測器資料腳位連接至4腳
#define WH_SCK_Pin 5   //定義重量感測器時序腳位連接至5腳
...
const int WH_factor = 4200; //定義重量感測器比例參數
...
void setup() 
{
  ...
  whScale.begin(WH_DT_Pin, 
                WH_SCK_Pin);   //初始化重量感測器物件
...
void loop() 
{
  ...
  if (delayRunning && (millis()- delayStart) >= delay_time)
  {
    ...
    //透過數字顯示器顯示重量值
    char line[6];
    dtostrf(weight, 3, 1, line);
    disp.displayString(line);
    //如果感測到重量,回傳至樹梅派
    if(weight > 5) {
